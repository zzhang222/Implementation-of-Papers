...
class Brain:
    ...
    def run_msnn(self):
        ...
        for i in range(self.iterations + 1):
            if self.batch_size is not None:
                ...
                loss = self.__criterion(self.net(self.data.X_train[mask]), self.data.h)
            else:
                loss = self.__criterion(self.net(self.data.X_train), self.data.h)
            if i % self.print_every == 0 or i == self.iterations:
                loss_test = self.__criterion(self.net(self.data.X_test), self.data.h)
    ...
    def run_rnn(self):
        ...
        for i in range(self.iterations + 1):
            if self.batch_size is not None:
                mask = np.random.choice(self.data.X_train.size(0), self.batch_size, replace=False)
                y_true = self.data.X_train[mask,1:]
                y_predict, new_state = self.net(self.data.X_train[mask,:-1])
                loss = self.__criterion(y_predict, y_true)
            else:
                y_true = self.data.X_train[:,1:]
                y_predict, new_state = self.net(self.data.X_train[:,:-1])
                loss = self.__criterion(y_predict, y_true)
            if i % self.print_every == 0 or i == self.iterations:
                X_predict = self.net.predict(self.data.X_test[:,:1], new_state, self.data.X_test.shape[1])
                loss_test = self.__criterion(self.data.X_test, X_predict)
                loss_history.append([i, loss.item(), loss_test.item()])
                print('{:<9}Train loss: {:<25}Test loss: {:<25}'.format(i, loss.item(), loss_test.item()), flush=True)
                if torch.any(torch.isnan(loss)):
                    self.encounter_nan = True
