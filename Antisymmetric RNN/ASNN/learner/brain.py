...
class Brain:
    ...
    def Init(cls, data, net, criterion, optimizer, lr, iterations, batch_size=None, 
             print_every=1000, save=False, callback=None, dtype='float', device='cpu', multi_gpu = False):
        cls.brain = cls(data, net, criterion, optimizer, lr, iterations, batch_size, 
                         print_every, save, callback, dtype, device, multi_gpu)
    ...
    @classmethod
    def Output(cls, data=False, best_model=True, loss_history=True, info=None, path=None, **kwargs):
    ...
    def run_rnn(self):
        ...
        for i in range(self.iterations + 1):
            if self.batch_size is not None:
                ...
                y_predict = self.net(self.data.X_train[mask])
                loss = self.__criterion(y_predict, self.data.y_train[mask])
            else:
                y_predict = self.net(self.data.X_train)
                loss = self.__criterion(y_predict, self.data.y_train)
            if i % self.print_every == 0 or i == self.iterations:
                with torch.no_grad():
                    if self.batch_size is not None:
                        num_batch = self.data.X_test.size(0) // self.batch_size
                        loss_test = 0
                        for j in range(num_batch):
                            y_test = self.net(self.data.X_test[j * self.batch_size : (j+1) * self.batch_size])
                            loss_test += self.__criterion(y_test, self.data.y_test[j * self.batch_size : (j+1) * self.batch_size]).detach()
                        loss_test = loss_test / num_batch
                    else: 
                        y_test = self.net(self.data.X_test)
                        loss_test = self.__criterion(y_test, self.data.y_test)
                ...
                if self.save:
                    ...
                    torch.save(self.net.state_dict(), 'model/model{}.pkl'.format(i))
    ...
    def __init_brain(self):
        ...
        if self.multi_gpu:
            ...
            self.net.module.device = self.device
            self.net.module.dtype = self.dtype
        else:
    ...
    def __init_optimizer(self):
        if self.optimizer == 'adam':
            ...
            self.__optimizer = torch.optim.RMSprop(self.net.parameters(), lr=self.lr)
        else:
